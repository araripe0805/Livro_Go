...
func main() {
	...
	//Go oferece dois tipos da dados de ponto flutuante: float32 e float64
	//float32 oferece em torno de 6 digitos decimais de precisão
	//Enquanto float64 oferece em torno de 15 digitos
	var a float32 = 2.98
	var b float64 = 3.198139189

	//Valores do tipo bool, ou booleano, tem 2 valores possiveis: true e false
	//As condições em instruções if e for são booleanas
	//operadores de comparação como == e < geram um resultado booleano
	//O operador unário ! é a negação lógica, portanto !true é false
	//Valores booleanos podem ser combinados com os operadores && (E) e || (OU)
	s := ""
	r := "a"
	if s == "" && r != "" {
		fmt.Println("teste")
